...
- 已新增 `src/codec/dy_grid.rs`：根据三定位点推算圆心、内外半径、边框状态、编码环和每环点数，并按极坐标采样得到 `DyGrid`；黑框版拆成 5 条编码环、1 个两段式外黑框和 2 条独立细环；外黑框按 `samples/两段外黑框.svg` 的 4 条切线生成两段完整扇环，其中 3 条可变切线用原图黑白边缘连续搜索；细环使用原图 Otsu 二值图按 720 角、5x7 专用采样核采样，120 点黑框版会在闭合前删掉徽标边缘 4 点以内短 run，再做小白缝闭合；右上徽标保留区内编码环保持空白，黑框版编码环不避让三牛眼白底，黑框版 72/120 点编码环使用不同徽标保留半径，120 点最外编码环额外剪掉紧邻徽标保留区的短 run，细环按 72/120 点版本使用不同徽标避让半径；黑框版点数在 72/120 中评分选择，`samples/黑框版2.jpg` 锁定为 120 点/环，旧 4 条标准编码环与 `samples/黑框版2.svg` 的 `g#c` 标准点位逐点一致；无框版固定 120 点/环、6 环
- 已接入应用流程和 UI：导入抖音码后自动识别、采样、生成 SVG 和右侧预览；抖音面板提供重新采样、显示差异、环数/点数/边框状态和差异像素数
- 已在 `src/vector/svg.rs` 支持抖音码 SVG、光栅预览和像素级差异预览；同环连续黑点会先合并成 run，黑框版把两段外黑框和两条细环输出到 `g#a`，72 点布局复用 `samples/黑框版1.svg` 的完整 `g#b`，120 点布局复用 `samples/黑框版2.svg` 的固定标记但去掉其中两个外框 path，并在黑框徽标内圆追加白底，再把 5 条编码环输出到最顶层 `g#c`；无框版输出封闭填充圆角弧条，单个独立点输出真实 `<circle>`；右上角徽标为黑外圈、白内圈，并嵌入 `samples/logo.svg` 的标准 logo path
- 已完成打包优化：`[profile.release]` 启用 `opt-level = "z"`、LTO、单 codegen unit、strip 和 `panic = "abort"`

**验证已通过**：
- `cargo fmt --check`
- `cargo clippy --no-deps -- -D warnings`
- `cargo test`（包含 `samples/黑框版*`、`samples/无框版*` 抖音标准样图回归、`samples/黑框版1.svg` 固定布局回归，以及 `samples/黑框版2.svg` 标准 SVG 逐点对比）
...
- 黑框版环数/半径从径向 profile 改为使用标准几何：主网格只保留 5 条编码环，包含 4 条已对齐 `samples/黑框版2.svg` 的编码环和靠近中心 Logo 的最内编码环；外黑框拆为 `DyOuterFrame` 两段完整扇环，两条细环拆为 `DyDecorativeRing`，它们都不参与编码和 72/120 点数判断
- 黑框版采样相位从径向黑度峰值改为再平移半个 cell，因为与 `黑框版2.svg` 逐点对比时旧相位整体偏 1 格；标准 SVG 的码点路径按 cell 中心落位，而 JPEG 黑度峰值更接近 cell 边界
- 黑框版采样阈值从 `>= 0.45` 改为 `>= 0.34` 的 3x3 投票，并在采样后删除“前一格白、后一格黑、3x3 黑度不超过 4/9”的弱前导端点；原因是 `>= 0.45` 会漏掉标准里的真实弱端点，而单纯低阈值会把 JPEG 边缘毛刺采成多余黑点；无框版仍保留 `>= 0.26`
...
1. `src/codec/dy_grid.rs`：
   ```rust
   pub struct DyParams {
      pub ring_count: u8,        // 黑框 5 条编码环 / 无框 6 环
...
2. 算法：
   ...
   - 环数：无框版固定 6 环；黑框版主网格为 5 条编码环，半径来自标准几何，并按三定位点距离缩放映射回输入图
   - 外黑框/细环：黑框版外黑框不参与编码和点数判断，单独存入 `DyOuterFrame`，按 4 条切线生成两段完整扇环，除右上徽标切线外的 3 条切线按原图边缘微调；两个细环单独存入 `DyDecorativeRing` 并按原图 720 角、5x7 专用核采样输出；点数和相位检测只使用前 4 条标准编码环
...
**步骤**：
1. `sample_dy(...) -> DyGrid` / `sample_dy_with_logos(...) -> DyGrid`：根据已检测参数生成同心环规格，并做角向相位搜索
2. 每个 (ring, point) 使用 3x3 极坐标采样投票；黑框版相位使用搜索结果 + 半个 cell，黑度比例 `>= 0.34` 判黑，并删除弱前导端点毛刺；无框版 `>= 0.26` 判黑；右上徽标和中心 Logo 覆盖区作为保留区跳过，黑框版编码环不跳过三牛眼白底，无框版仍跳过三牛眼。黑框版和无框版编码环在右上徽标保留区内一律保持空白，不做强黑度补采；其中 120 点黑框版徽标保留半径为 `1.12 * badge.radius`，72 点黑框版和无框版为 `1.04 * badge.radius`；120 点黑框版最外编码环会额外处理紧邻徽标保留区的 2-cell 短 run，但只删除其中距离比例 `<= 1.20` 的内部徽标边缘 cell
3. 注意：黑框版外黑框不是采样碎片，而是两段完整外框；右上徽标切线固定，其余 3 条切线按原图黑白边缘连续搜索微调，当前使用 0.5° 前后探针。两个细环用原图 Otsu 二值图和 720 角采样保留细线，避开右上徽标区域，并做 5x7 细环核、小白缝闭合和短黑噪删除；72 点细环使用放宽徽标避让以贴到徽标圆框，120 点细环保持严格避让以匹配 `黑框版2.svg`，并在闭合白缝前删除徽标边缘 4 点以内短 run，防止徽标圆框被接成细环；编码环用清理后的二值图
4. SVG：同一环连续黑点先合并成 run；黑框版把两段外黑框和细环输出到 `g#a`，按 72/120 布局追加对应标准 SVG 的固定 `g#b`，最后把码点输出到最顶层 `g#c`，让外框/细环被白底遮挡但编码环不被白底遮挡；无框版输出闭合填充圆角弧条，单个独立点输出 `<circle>`；全程不用描边
5. 徽标：黑框版右上徽标和三牛眼直接来自 `samples/黑框版1.svg` 或 `samples/黑框版2.svg` 标准 `g#b` 的固定造型，但外黑框 path 不复用；导出时在黑框徽标黑外圈 path 后追加白色内圆，再绘制 logo path，防止下层细环透进徽标内圆；无框版右上徽标为黑外圈、白内圈，并嵌入 `samples/logo.svg` 的标准 logo path；徽标检测失败时根据三定位点估算位置

**验收**：完整流程跑通；`samples/黑框版2.jpg` 的旧 4 条标准编码环 4x120 采样布尔网格与 `samples/黑框版2.svg` 的 `g#c` 逐点一致；`samples/黑框版2.svg` 的 `g#a` 会被解析成 720 角标准覆盖，用于约束两条细环 run 数和角段覆盖，`g#b` 前两个外框 path 会被解析成标准切线角度，用于约束外框切线误差不超过 2°；`samples/黑框版1.jpg` 输出直接复用 `samples/黑框版1.svg` 的完整 `g#b`，但不固定复用 `g#a`；外黑框输出为两段完整外框，两条细环能从原图采到且不会过度碎片化，最内环能从原图采到黑点；`黑框版1.jpg` 细环能采到徽标外框接触段，`黑框版1漏采点标注.svg` 中靠近徽标的 72 点里层编码环能采回，`黑框版2漏采点标注.svg`/`黑框版3多采漏采点位标注.svg`/`黑框版3新问题.svg`/`黑框版4多采漏采点位标注.svg` 中红圈半径内所有编码 cell 为黑且由最终 `g#c` 编码环 run 覆盖、蓝圈不与编码环/细环/外框动态输出重叠，根目录 `黑框版4.jpg` 左上牛眼附近编码点不会被牛眼白底保留区误删，所有黑框版和无框版编码环在右上徽标保留区内保持空白
