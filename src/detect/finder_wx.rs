...
mod tests {
    ...
    #[test]
    fn finds_three_synthetic_bullseyes() {
        let mut bin = BinaryImage::new(420, 420, vec![255; 420 * 420]);
        draw_bullseye(&mut bin, 90, 90, 18);
        draw_bullseye(&mut bin, 330, 90, 18);
        draw_bullseye(&mut bin, 90, 330, 18);

        let finders = find_wx_finders(&bin);
        let selected = select_wx_finders(&finders).unwrap();

        assert_eq!(selected.len(), 3);
    }

    #[test]
    fn normalizes_near_axis_finder_alignment() {
        let finders = [
            WxFinder {
                cx: 142.0,
                cy: 138.0,
            ...
            WxFinder {
                cx: 492.071_320_182_094_1,
                cy: 136.392_261_001_517_45,
                r_outer: 27.25,
            },
            WxFinder {
                cx: 143.392_261_001_517_45,
                cy: 485.071_320_182_094_1,
                r_outer: 27.25,
            },
        ];

        let normalized = normalize_wx_finder_triplet(finders);

        assert!((normalized[1].cy - normalized[0].cy).abs() < 1e-6);
        assert!((normalized[2].cx - normalized[0].cx).abs() < 1e-6);
        assert!((normalized[1].cx - finders[1].cx).abs() < 0.01);
        assert!((normalized[2].cy - finders[2].cy).abs() < 0.01);
    }
