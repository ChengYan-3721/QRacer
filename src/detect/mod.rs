...
pub fn detect_kind(bin: &BinaryImage) -> CodeKind {
    detect_kind_impl(bin, DetectionHints::default())
}
...
mod tests {
    ...
    fn detect_kind_returns_wx_for_standard_sample() {
        ...
        assert_eq!(detect_kind_with_image(&img, &bin), CodeKind::WxMiniprogram);
    }

    #[test]
    fn detect_kind_returns_wx_for_mini_program_samples() {
        for path in sample_paths(&["小程序码"]) {
            ...
            assert_eq!(
                detect_kind_with_image(&img, &bin),
                CodeKind::WxMiniprogram,
                "{} signature={signature:?} hints={hints:?} qr={has_qr} wx={has_wx} dy={has_dy}",
                path.display(),
            );
        }
    }
    ...
    #[test]
    fn detect_kind_returns_douyin_for_douyin_samples() {
