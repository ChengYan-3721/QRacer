...
mod tests {
    ...
    fn finds_three_finders_on_synthetic_qr() {
        let qr = QrCode::encode_segments_advanced(
            &qrcodegen::QrSegment::make_segments("qracer finder test"),
            QrCodeEcc::Medium,
            Version::new(5),
            Version::new(5),
            None,
        ...
        assert!(finders.len() >= 3, "{finders:?}");
        assert!(
            select_qr_finder_triplet(&bin, &finders).is_some(),
            "{finders:?}"
        );
    }

    #[test]
    fn ignores_one_axis_pattern_without_vertical_match() {
        let mut data = vec![255; 60 * 30];
        let y = 15;
        for x in 10..15 {
            data[y * 60 + x] = 0;
        }
        ...
        let bin = BinaryImage::new(60, 30, data);

        assert!(find_qr_finders(&bin).is_empty());
    }

    #[test]
    fn selects_wide_qr_triangle_over_data_area_candidate() {
        let qr = QrCode::encode_segments_advanced(
            &qrcodegen::QrSegment::make_segments("qracer finder test"),
            QrCodeEcc::Medium,
            Version::new(5),
        ...
        let actual = [
            ...
            QrFinder {
                cx: (border + size - 3.5) * scale,
                cy: (border + 3.5) * scale,
                module: scale,
            },
            QrFinder {
                cx: (border + 3.5) * scale,
                cy: (border + size - 3.5) * scale,
                module: scale,
            },
        ];
        let false_positive = QrFinder {
            cx: (border + size * 0.72) * scale,
            cy: (border + size * 0.55) * scale,
            module: scale,
        };
        let candidates = [actual[0], false_positive, actual[1], actual[2]];

        let bin = render_qr_binary(&qr, scale as u32, border as u32);
        let selected = select_qr_finder_triplet(&bin, &candidates).unwrap();

        for expected in actual {
            assert!(selected.iter().any(|finder| {
